...
import mod.weaponery.init.WeaponeryModEntities;

import java.util.Random;

@OnlyIn(value = Dist.CLIENT, _interface = ItemSupplier.class)
public class PistolEntity extends AbstractArrow implements ItemSupplier {
	public PistolEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		super(WeaponeryModEntities.PISTOL, world);
	}
	...
	public static PistolEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		PistolEntity entityarrow = new PistolEntity(WeaponeryModEntities.PISTOL, entity, world);
		...
		world.playSound((Player) null, entity.getX(), entity.getY(), entity.getZ(),
				ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("weaponery:pistol_shooting")), SoundSource.PLAYERS, 1,
				1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static PistolEntity shoot(LivingEntity entity, LivingEntity target) {
		PistolEntity entityarrow = new PistolEntity(WeaponeryModEntities.PISTOL, entity, entity.level);
		double d0 = target.getY() + (double) target.getEyeHeight() - 1.1;
		double d1 = target.getX() - entity.getX();
		double d3 = target.getZ() - entity.getZ();
		entityarrow.shoot(d1, d0 - entityarrow.getY() + Math.sqrt(d1 * d1 + d3 * d3) * 0.2F, d3, 15f * 2, 12.0F);
		entityarrow.setSilent(true);
